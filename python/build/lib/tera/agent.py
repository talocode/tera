...
BOT_TOKEN = os.environ.get("TERAAI_BOT_TOKEN") or os.environ.get("TELEGRAM_BOT_TOKEN")
...
if not BOT_TOKEN: raise SystemExit("TERAAI_BOT_TOKEN not set")
...
SENSITIVE_KEYS = [
    ...
    os.environ.get("TALOCODE_API_KEY", ""),
    os.environ.get("SUPABASE_SERVICE_ROLE_KEY", ""),
    os.environ.get("SUPABASE_ANON_KEY", ""),
]

def sanitize(text):
    """Strip secrets/tokens from error messages before sending to users."""
    s = str(text)
    if "Network is unreachable" in s or "ConnectionError" in s or "Connection refused" in s:
    ...
    s = re.sub(r'eyJ[A-Za-z0-9_-]+\.eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+', '[REDACTED]', s)
    return s

FIRECRAWL_URL = "https://api.firecrawl.dev/v1"

def firecrawl_scrape(url):
    """Scrape a URL to markdown using Firecrawl API."""
    if not FIRECRAWL_KEY:
        return {"error": "FIRECRAWL_API_KEY not configured"}
    try:
        r = requests.post(f"{FIRECRAWL_URL}/scrape",
            headers={"Authorization": f"Bearer {FIRECRAWL_KEY}", "Content-Type": "application/json"},
            json={"url": url, "formats": ["markdown"], "onlyMainContent": True},
            timeout=30)
        if r.status_code == 200:
            data = r.json()
            return {"markdown": data.get("data", {}).get("markdown", ""), "success": True}
        return {"error": f"Scrape failed: {r.status_code}"}
    except Exception as e:
        return {"error": sanitize(e)}

def firecrawl_search(query, max_results=5):
    """Search the web using Firecrawl API."""
    if not FIRECRAWL_KEY:
        return {"error": "FIRECRAWL_API_KEY not configured"}
    try:
        r = requests.post(f"{FIRECRAWL_URL}/search",
            headers={"Authorization": f"Bearer {FIRECRAWL_KEY}", "Content-Type": "application/json"},
            json={"query": query, "limit": max_results, "scrapeOptions": {"formats": ["markdown"], "onlyMainContent": True}},
            timeout=20)
        if r.status_code == 200:
            data = r.json()
            results = []
...
def format_web_context(data):
    """Format web context response for Telegram."""
    if "error" in data:
...
CAPABILITIES = {
    ...
    "webcontext": {
        "name": "Web Context",
        "desc": "Get web context, scrape URLs, search with live web data via Context.dev",
        "triggers": ["context", "web context", "scrape url", "get page", "fetch website", "what does this site say", "web info"],
    },
    "browse": {
        "name": "Agent Browser",
    ...
    "cliploop": {
        ...
        "triggers": ["clip", "short", "reel", "tiktok", "youtube short"],
        "api": {"path": "/v1/cliploop/brief/generate", "keyed": True, "credit": 15},
    },
    "videolane": {
        "name": "VideoLane",
        "desc": "Produce full videos — scripts, voiceover, captions, thumbnails, YouTube metadata, and upload packages",
        "triggers": ["video", "produce video", "make video", "video production", "video lane", "tutorial video", "demo video", "youtube video"],
    },
    "content": {
        "name": "UGC Lane",
...
def handle_capability(chat_id, key, user_text):
    ...
    if key == "webcontext":
        if not FIRECRAWL_KEY:
            tg("sendMessage", {"chat_id": chat_id, "text": "⚠️ FIRECRAWL_API_KEY not configured. Add it to `.env` to use web context.", "parse_mode": "Markdown"})
            return True
        tg("sendChatAction", {"chat_id": chat_id, "action": "typing"})
        try:
            text = user_text.lower()
            if any(w in text for w in ["scrape", "fetch", "get page", "what does", "site say", "contents of"]):
                url_match = re.search(r'https?://[^\s]+', user_text)
                if url_match:
                    url = url_match.group(0)
                    tg("sendMessage", {"chat_id": chat_id, "text": f"🔍 Scraping {url}..."})
                    result = firecrawl_scrape(url)
                    response = format_web_context(result)
                    tg("sendMessage", {"chat_id": chat_id, "text": response, "parse_mode": "Markdown"})
                else:
                    tg("sendMessage", {"chat_id": chat_id, "text": "Please provide a URL to scrape. Example: *scrape https://example.com*", "parse_mode": "Markdown"})
            else:
                query = user_text
                for w in ["context", "web context", "search", "find"]:
                    query = query.replace(w, "").strip()
                if not query:
                    query = user_text
                tg("sendMessage", {"chat_id": chat_id, "text": f"🔍 Searching: {query}..."})
                result = firecrawl_search(query)
                response = format_web_context(result)
                tg("sendMessage", {"chat_id": chat_id, "text": response, "parse_mode": "Markdown"})
        except Exception as e:
            tg("sendMessage", {"chat_id": chat_id, "text": f"Error: {sanitize(e)}"})
        return True
    ...
    if key == "videolane":
        ...
        try:
            ...
            # Check if videolane server is running
            server_running = False
            try:
                health = req.get("http://localhost:3110/health", timeout=2)
            ...
            if server_running:
                ...
                result = req.post("http://localhost:3110/v1/videolane/plan",
                    json={"input": user_text}, timeout=60).json()
                tg("sendMessage", {"chat_id": chat_id, "text": f"🎬 VideoLane plan:\n\n```json\n{json.dumps(result, indent=2)[:3000]}```", "parse_mode": "Markdown"})
            else:
                # Server not running — start it and try again
                import subprocess
                subprocess.Popen(
                    ["node", "-e", "const { createServer } = require('./dist/api/server.js'); createServer(3110);"],
                    cwd="/workspace/projects/videolane",
                ...
                try:
                    ...
                    tg("sendMessage", {"chat_id": chat_id, "text": "🎬 VideoLane server started but couldn't connect. Try again in a moment."})
        except Exception as e:
            tg("sendMessage", {"chat_id": chat_id, "text": f"Error: {sanitize(e)}"})
        return True
    ...
    if key == "email":
        ...
        try:
            ...
            tg("sendMessage", {"chat_id": chat_id, "text": f"Error sending email: {sanitize(e)}"})
        return True
    ...
    if key == "pdf":
        ...
        try:
            ...
            tg("sendMessage", {"chat_id": chat_id, "text": f"Error generating PDF: {sanitize(e)}"})
        return True
    ...
    if key == "calendar":
        ...
        try:
            ...
            tg("sendMessage", {"chat_id": chat_id, "text": f"Calendar error: {sanitize(e)}"})
        return True
    ...
    if key == "memory":
        ...
        try:
            ...
            tg("sendMessage", {"chat_id": chat_id, "text": f"Memory error: {sanitize(e)}"})
        return True
    ...
    if key == "voice":
        ...
        try:
            ...
            tg("sendMessage", {"chat_id": chat_id, "text": f"Voice error: {sanitize(e)}"})
        return True
    ...
    if "api" in cap:
        ...
        try:
            ...
            tg("sendMessage", {"chat_id": chat_id, "text": f"Error calling {cap['name']}: {sanitize(e)}"})
        return True
    return False
...
def main():
    ...
    while True:
        try:
            ...
            print(f"Network error: {sanitize(e)}")
            time.sleep(10)
            continue
        ...
        for update in result.get("result", []):
            ...
            try:
                ...
                err = f"Error: {sanitize(e)}"
                try:
                    tg("sendMessage", {"chat_id": chat_id, "text": err})
                except Exception:
                    print(f"Failed to send error to {chat_id}: {err}")
                print("Error:", err)
