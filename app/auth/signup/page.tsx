    ...
    <div className="tera-page flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-[32px] border border-tera-border bg-tera-panel/82 p-6 shadow-soft-lg backdrop-blur-2xl md:p-8 lg:p-10">
        ...
        <div className="mt-8 text-center">
          <p className="tera-eyebrow">Get started</p>
          <h1 className="mt-3 text-2xl font-semibold tracking-[-0.03em] text-tera-primary md:text-3xl">Create your Tera account</h1>
          <p className="mt-3 text-sm leading-7 text-tera-secondary md:mt-4">
            Start with the free plan and move into the same dark workspace used across chat, tools, notes, and history.
          </p>
        </div>
        ...
        <div className="tera-card-subtle mt-7 space-y-3 p-4 md:mt-8 md:p-5">
          ...
          <div className="flex items-start gap-3 text-sm leading-7 text-tera-secondary">
            <span className="mt-2 h-2.5 w-2.5 rounded-full bg-tera-neon" />
            <span>Upgrade later for larger limits, research mode, and analytics.</span>
          </div>
        </div>

        <p className="mt-8 text-center text-sm text-tera-secondary">
          Already have an account?{' '}
          <Link href="/auth/signin" className="text-tera-neon underline decoration-tera-neon/30 underline-offset-4">
            Sign in
          </Link>
        </p>

        <p className="mt-4 text-center text-xs leading-6 text-tera-secondary">
          By continuing, you agree to the <Link href="/terms" className="text-tera-neon">Terms</Link> and <Link href="/privacy" className="text-tera-neon">Privacy Policy</Link>.
        </p>
      </div>
    </div>
  )
